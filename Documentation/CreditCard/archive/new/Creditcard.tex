...
\begin{titlepage}
  ...
  Santosh Kumar Shah\\[0.1cm]
  Surya Chand Yadav\\[0.1cm]
  Salim Shrestha\\[1.1cm]
...
\begin{titlepage}
  ...
  {\Large Surya Chand Yadav}\\[0.35cm]
  {\Large Santosh Kumar Shah}\\[0.35cm]
  {\Large Salim Shrestha}\\[1.20cm]
...
\vspace{1.2cm}
\noindent\textbf{Student Declaration and Signatures}

\vspace{0.7cm}
\renewcommand{\arraystretch}{2.2}
\begin{tabular}{>{\bfseries}p{4.6cm} p{5.3cm} p{3.4cm}}
\toprule
Name & Signature & Date \\
\midrule
Santosh Kumar Shah & \rule{4.8cm}{0.4pt} & \projectdate \\
Surya Chand Yadav & \rule{4.8cm}{0.4pt} & \projectdate \\
Salim Shrestha & \rule{4.8cm}{0.4pt} & \projectdate \\
\bottomrule
\end{tabular}
...
\begin{tikzpicture}[
    ...
    flowstep/.style={rectangle, rounded corners, draw=black, fill=gray!10, align=center, minimum width=0.82\textwidth, minimum height=0.8cm},
    arrow/.style={-{Latex[length=2.2mm]}, thick}
]
\node[flowstep] (input) {Dashboard/API transaction input};
\node[flowstep, below=of input] (validate) {Validate fields and normalize transaction metadata};
\node[flowstep, below=of validate] (behavior) {Analyze customer behavior profile};
\node[flowstep, below=of behavior] (features) {Engineer 15 features and apply StandardScaler};
\node[flowstep, below=of features] (model) {Random Forest predicts fraud probability};
\node[flowstep, below=of model] (risk) {Apply threshold, calculate risk score, assign risk level};
\node[flowstep, below=of risk] (store) {Create alerts when needed, update profile, save transaction};
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    node distance=1.45cm and 1.65cm,
    box/.style={rectangle, rounded corners, draw=black, fill=gray!10, align=center, minimum width=3.35cm, minimum height=1.05cm},
    data/.style={cylinder, shape border rotate=90, aspect=0.25, draw=black, fill=gray!15, align=center, minimum width=3.15cm, minimum height=1.15cm},
    arrow/.style={-{Latex[length=2.2mm]}, thick}
]
\node[box] (browser) {Browser Dashboard\\\path{templates/index.html}};
\node[box, below=of browser] (flask) {Flask Application\\\path{app.py}};
\node[box, left=of flask] (batch) {CSV Upload\\Report Export};
\node[box, right=of flask] (features) {Feature Engineering\\\path{feature_engineering.py}};
\node[data, below left=1.25cm and 0.8cm of flask] (artifacts) {Model Artifacts\\\path{fraud_model.pkl}\\\path{scaler.pkl}};
\node[data, below right=1.25cm and 0.8cm of flask] (store) {Runtime Store\\\path{instance/data_store.json}};

\draw[arrow] (browser) -- (flask);
\draw[arrow] ([xshift=-0.35cm]flask.north) -- ([xshift=-0.35cm]browser.south);
\draw[arrow] (batch) -- (flask);
\draw[arrow] (flask) -- (batch);
\draw[arrow] (flask) -- (features);
\draw[arrow] (features) -- (flask);
\draw[arrow] (flask) -- (artifacts);
\draw[arrow] (artifacts) -- (flask);
\draw[arrow] (flask) -- (store);
\draw[arrow] (store) -- (flask);
\end{tikzpicture}
}
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    node distance=1.05cm and 0.9cm,
    process/.style={rectangle, rounded corners, draw=black, fill=gray!10, align=center, minimum width=2.9cm, minimum height=0.95cm},
    note/.style={rectangle, rounded corners, draw=black, fill=white, align=center, minimum width=2.85cm, minimum height=0.85cm, font=\small},
    store/.style={cylinder, shape border rotate=90, aspect=0.25, draw=black, fill=gray!15, align=center, minimum width=2.7cm, minimum height=0.95cm},
    labeltext/.style={fill=white, inner sep=1.5pt, font=\small},
    arrow/.style={-{Latex[length=2.2mm]}, thick},
    feedback/.style={-{Latex[length=2.2mm]}, thick, dashed}
]
\node[process] (input) {Single form\\or CSV row};
\node[process, right=of input] (parse) {Parse and\\validate};
\node[process, right=of parse] (profile) {Load customer\\profile};
\node[process, right=of profile] (engineer) {Behavior and\\feature engineering};
\node[process, below=1.25cm of engineer] (score) {Scale features\\score model};
\node[process, left=of score] (decision) {Risk level\\verification status};
\node[process, left=of decision] (output) {Dashboard result\\or CSV report};

\node[store, below=1.35cm of output] (transactions) {Transactions};
\node[store, below=1.35cm of decision] (alerts) {Alerts};
\node[store, below=1.35cm of score] (profiles) {Profiles};
\node[note, right=1.0cm of profiles] (history) {Profile history\\for next request};

\draw[arrow] (input) -- (parse);
\draw[arrow] (parse) -- (profile);
\draw[arrow] (profile) -- (engineer);
\draw[arrow] (engineer) -- (score);
\draw[arrow] (score) -- (decision);
\draw[arrow] (decision) -- (output);

\draw[arrow] (decision) -- node[labeltext, left]{save scored record} (transactions);
\draw[arrow] (decision) -- node[labeltext, right]{if high risk/fraud} (alerts);
\draw[arrow] (score) -- node[labeltext, right]{update history} (profiles);
\draw[feedback] (profiles) -- (history);
\draw[feedback] (history.north) |- (profile.south);
\end{tikzpicture}
}
\caption{Transaction Data Flow}
\label{fig:data-flow}
\end{figure}

\section{Sequence Design}
Figure~\ref{fig:sequence-diagram} shows the interaction sequence for a single transaction prediction request. The sequence begins with transaction submission from the dashboard and ends when the dashboard receives the prediction result and risk information.

\begin{figure}[H]
\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    participant/.style={rectangle, draw=black, fill=gray!10, align=center, minimum width=2.45cm, minimum height=0.75cm},
    lifeline/.style={dashed, gray},
    msg/.style={-{Latex[length=2mm]}, thick},
    returnmsg/.style={-{Latex[length=2mm]}, thick, dashed},
    note/.style={font=\small, fill=white, inner sep=1.5pt}
]
\node[participant] (op) at (0,0) {Operator};
\node[participant] (ui) at (3,0) {Dashboard};
\node[participant] (app) at (6,0) {Flask App};
\node[participant] (feat) at (9,0) {Feature\\Engineering};
\node[participant] (model) at (12,0) {Scaler /\\Model};
\node[participant] (store) at (15,0) {JSON\\Store};

\foreach \x in {op,ui,app,feat,model,store} {
    \draw[lifeline] (\x.south) -- ++(0,-7.2);
}

\draw[msg] (0,-0.9) -- node[note, above]{enter transaction} (3,-0.9);
\draw[msg] (3,-1.7) -- node[note, above]{POST /predict} (6,-1.7);
\draw[msg] (6,-2.5) -- node[note, above]{load profile and transaction} (15,-2.5);
\draw[returnmsg] (15,-3.1) -- node[note, above]{profile history} (6,-3.1);
\draw[msg] (6,-3.9) -- node[note, above]{engineer features} (9,-3.9);
\draw[returnmsg] (9,-4.5) -- node[note, above]{15 model features} (6,-4.5);
\draw[msg] (6,-5.3) -- node[note, above]{scale and predict} (12,-5.3);
\draw[returnmsg] (12,-5.9) -- node[note, above]{fraud probability} (6,-5.9);
\draw[msg] (6,-6.7) -- node[note, above]{save result, alert, profile} (15,-6.7);
\draw[returnmsg] (6,-7.5) -- node[note, above]{label, risk score, signals} (3,-7.5);
\draw[returnmsg] (3,-8.2) -- node[note, above]{display result} (0,-8.2);
\end{tikzpicture}
}
\caption{Single Transaction Prediction Sequence}
