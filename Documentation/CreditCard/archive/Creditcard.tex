...
\draw[arrow] (outputs) to[bend left=18] node[labeltext, below]{review and verification actions} (operator);

\draw[arrow] (model) -- node[labeltext, right]{scaler and trained model} (pipeline);
\draw[arrow] (model) -| node[labeltext, below]{fraud probability support} (risk);
\draw[arrow] (runtime) -- node[labeltext, right]{history for behavior profile} (risk);
\draw[arrow] (risk) -- node[labeltext, above]{save scored transactions and alerts} (runtime);
\end{tikzpicture}
}
