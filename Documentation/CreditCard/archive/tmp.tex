...
%---------diagram start (FIXED: no overlap)---------
\begin{figure}[H]
\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    node distance=0.95cm and 0.65cm,
    external/.style={rectangle, draw=black, fill=white, align=center, minimum width=2.35cm, minimum height=0.85cm},
    process/.style={rectangle, rounded corners, draw=black, fill=gray!10, align=center, minimum width=2.45cm, minimum height=0.9cm},
    store/.style={cylinder, shape border rotate=90, aspect=0.25, draw=black, fill=gray!15, align=center, minimum width=2.55cm, minimum height=0.95cm},
    labeltext/.style={fill=white, inner sep=1.5pt, font=\scriptsize},
    arrow/.style={-{Latex[length=2.2mm]}, thick}
]
\node[external] (operator) at (0,0) {Operator / Admin};
\node[process] (dashboard) at (3,0) {Browser\\Dashboard};
\node[process] (backend) at (6,0) {Flask\\Backend};
\node[process] (pipeline) at (9,0) {Validation and\\Feature Engineering};
\node[process] (risk) at (12,0) {Prediction, Risk\\and Alert Logic};
\node[external] (outputs) at (15,0) {Dashboard Results\\CSV Reports};

\node[store] (model) at (9,-2.0) {Model Artifacts\\\path{fraud_model.pkl}\\\path{scaler.pkl}};
\node[store] (runtime) at (12,-2.0) {Runtime Store\\Transactions\\Alerts\\Profiles};

% Horizontal arrows
\draw[arrow] (operator) -- node[labeltext, above] {transaction / CSV input} (dashboard);
\draw[arrow] (dashboard) -- node[labeltext, above] {HTTP request} (backend);
\draw[arrow] (backend) -- node[labeltext, above] {validated request data} (pipeline);
\draw[arrow] (pipeline) -- node[labeltext, above] {engineered features and behavior signals} (risk);
\draw[arrow] (risk) -- node[labeltext, above] {label, risk score, alert status} (outputs);
\draw[arrow] (outputs) to[bend left=18] node[labeltext, below, yshift=-2pt] {review and verification actions} (operator);

% Vertical arrows: model -> pipeline (clear left side)
\draw[arrow] (model) -- node[labeltext, left] {scaler and trained model} (pipeline);

% Arrow from model to risk: curved to avoid overlap with runtime->risk line
\draw[arrow] (model) to[bend right=25] node[labeltext, fill=white, near start] {fraud probability support} (risk);

% Vertical arrows between runtime and risk (clear left/right sides)
\draw[arrow] (runtime) -- node[labeltext, left] {history for behavior profile} (risk);
\draw[arrow] (risk) -- node[labeltext, right, xshift=2pt] {save scored transactions and alerts} (runtime);
