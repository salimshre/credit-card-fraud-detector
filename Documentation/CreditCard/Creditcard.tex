...
% Vertical arrows: model -> pipeline (left side to avoid overlap)
\draw[arrow] (model) -- node[labeltext, left] {scaler and trained model} (pipeline);

% Arrow from model to risk: split into two segments to place label clearly
\draw[arrow] (model) -- (12,-2) -- (risk);
\node[labeltext, fill=white] at (12,-1.2) {fraud probability support};

% Vertical arrows between runtime and risk
