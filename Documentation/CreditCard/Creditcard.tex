...
\begin{titlepage}
    ...
    {\Large on}\\[0.3cm]
    {\Large \textbf{\projecttitle : \projectsubtitle}}\\[0.7cm]
    {\large Submitted in partial fulfillment of the requirement of
    Computer Project-II (BCE5017)}\\[0.4cm]
    {\Large of}\\[0.4cm]
    {BACHELOR IN COMPUTER ENGINEERING}\\[0.8cm]
    {\large \textbf{Submitted to}}\\[0.2cm]
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.35\textwidth]{assets/logo.jpg}
    \end{figure}
    {\large Purbanchal University}\\[0.15cm]
    {\large Biratnagar, Nepal}\\[0.9cm]
    {\large \textbf{Submitted by:}}\\[0.2cm]
    {\large Santosh Kumar Shah (751604)}\\
    {\large Surya Chand Yadav (752074)}\\
    {\large Salim Shrestha (751600)}\\[1cm]
    {\large \textbf{\MakeUppercase{\college}}}\\[0.15cm]
...
\begin{titlepage}
    ...
    {\large Santosh Kumar Shah (751604)}\\
    {\large Surya Chand Yadav (752074)}\\
    {\large Salim Shrestha (751600)}\\[0.9cm]
    {\large \textbf{Project Supervisor}}\\[0.2cm]
    {\large \supervisor}\\[0.9cm]
    {\large \textbf{\MakeUppercase{\college}}}\\[0.15cm]
    {\large \collegeaddress}\\
    \vfill
    {\large \textbf{\projectdate}}
...
\section{Limitations}
\begin{itemize}
    \item The repository uses SQLite (\path{instance/fraud_shield.db}) for runtime persistence; \path{data_store.json} is retained only as a legacy one-time migration source and is not the active store.
    \item Authentication uses bcrypt-hashed credentials stored in SQLite with rate-limited login attempts; role-based access control and multi-factor authentication are not yet implemented.
    \item The dataset in the repository is synthetic and may not capture all real banking fraud patterns.
    \item Model drift, audit logging, encryption in transit, and role-based access control are outside the current implementation.
...
Flask-Bcrypt & Secure hashing and verification of operator login passwords. \\ \hline
Flask-Limiter & Rate limiting on the login endpoint to reduce brute-force attempts. \\ \hline
...
SQLite & Relational database for transactions, alerts, behavior profiles, and users. \\ \hline
...
\path{instance/fraud_shield.db} & SQLite database for transactions, alerts, behavior profiles, and users (primary runtime persistence). \\ \hline
\path{instance/data_store.json} & Legacy JSON state file, imported once into SQLite if the database is empty on startup. \\ \hline
\end{tabular}
\end{table}
...
GET & \path{/health} & Return application status, threshold, model files, model feature count, and a list of application capabilities. \\ \hline
...
The application stores runtime state in memory (\path{TRANSACTIONS}, \path{ALERTS}, \path{BEHAVIOR_PROFILES}) and persists it to an SQLite database at \path{instance/fraud_shield.db}, defined in \path{storage.py}. The database contains four tables: \path{transactions}, \path{alerts}, \path{behavior_profiles}, and \path{users}. The \path{users} table stores bcrypt-hashed passwords used during login. If a legacy root-level \path{data_store.json} file exists and the database is empty, the application performs a one-time migration of its contents into SQLite; subsequent reads and writes use the database exclusively. Behavior profiles contain transaction counts, amount statistics, recent timestamps, known devices, known countries, and known merchant categories.
...
The project is implemented in Python. The required dependencies are listed in \path{requirements.txt}: Flask, Flask-Bcrypt, Flask-Limiter, Pandas, NumPy, Scikit-learn, and Joblib. The implementation is organized into separate files for the Flask application, feature engineering, model training, synthetic data generation, smoke testing, templates, and saved model artifacts.

\section{Authentication}
The application uses session-based authentication. Operator credentials are stored in a SQLite \path{users} table with bcrypt-hashed passwords, seeded on first startup from the \path{DEFAULT_ADMIN_PASSWORD} environment variable (falling back to a default value with a logged warning if unset). The default username, and the Flask \path{SECRET_KEY}, can be configured through environment variables; the application refuses to start if \path{SECRET_KEY} is missing or left at its insecure default. Login attempts are rate-limited to 5 per minute, and failed logins return a generic error message to avoid revealing whether a given username exists. Protected endpoints use the \path{require_auth} decorator, which returns HTTP 401 for unauthenticated requests.
...
Runtime persistence limitation & \textbullet\ Migrated from JSON-only storage to an SQLite database, with a one-time legacy JSON import for backward compatibility. \\ \hline
...
\begin{itemize}
    ...
    \item Add role-based access control for multiple operator permission levels.
\end{itemize}
